...
export class CustomersService {
  ...
  static async create(data: { name: string; phone: string; email?: string; address?: string; notes?: string }, userId: number) {
    ...
    const res = await dbService.execute(
      `INSERT INTO customers (name, phone, email, address, notes, status, total_visits, total_spent)
       VALUES (?, ?, ?, ?, ?, 'ACTIVE', 0, 0.0)`,
  ...
  static async update(id: number, data: { name?: string; phone?: string; email?: string; address?: string; notes?: string; status?: string }, userId: number) {
    ...
    await dbService.execute(
      `UPDATE customers
       ...
       WHERE id = ?`,
      [data.name, data.phone, data.email, data.address, data.notes, data.status, id]
    );
