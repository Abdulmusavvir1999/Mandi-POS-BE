  ...
  static async create(data: {
    ...
    return await dbService.transaction(async () => {
      ...
      for (const item of data.items) {
        const product = await dbService.queryOne<{ id: number; name: string; selling_price: number }>(
          'SELECT id, name, selling_price FROM products WHERE id = ?',
          [item.productId]
        );
        if (!product) {
        ...
        const price = item.unitPrice !== undefined ? item.unitPrice : product.selling_price;
