    ...
    const [items, totals, byCategory] = await Promise.all([
      dbService.query(
        `SELECT
           ...
           (si.current_quantity * COALESCE(NULLIF((SELECT MIN(pv.selling_price) FROM product_variants pv WHERE pv.product_id = p.id), 0), si.average_unit_price)) AS value_at_retail,
           CASE
